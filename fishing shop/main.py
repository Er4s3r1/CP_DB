...
def main():
    if not st.session_state.logged_in:
        ...
        with tab_register:
            ...
            if st.button("Зарегистрироваться"):
                if register(username, email, password):
                    st.success("Вы успешно зарегестрировались! Теперь можете войти в систему.")
                else:
        ...
        if role == 1:  # Админ
            page = st.sidebar.selectbox("Страницы", ["Товары", "Добавить товар", "Добавить категорию", "Заказы"])
        else:
            page = st.sidebar.selectbox("Страницы", ["Товары", "Корзина", "Мои заказы"])
...
def add_category():
    ...
    category_descrition = st.text_input("Описание категории")
            ...
            try:
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        (category_name, category_descrition),
                    )
                    conn.commit()
